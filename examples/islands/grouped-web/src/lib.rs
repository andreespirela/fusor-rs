...
fusor_islands::export!(
    fusor_islands::browser::Unit::new()
        .entry::<catalog_types::GroupedCart, catalog_views::CartView>(|_, props| {
            catalog_views::CartView::new(props)
        })
        .entry::<catalog_types::GroupedDesigner, catalog_designer_views::DesignerView>(
            |_, props| catalog_designer_views::DesignerView::new(props)
        )
);
fusor::bindings!(app);
...
pub async fn exercise_control(prefetch: bool) -> Result<(), wasm_bindgen::JsValue> {
    let owner = fusor::Owner::new();
    let target =
        fusor_islands::browser::get::<catalog_types::GroupedDesigner>(&owner.handle(), "designer")
            .map_err(|error| wasm_bindgen::JsValue::from_str(&error.to_string()))?;
    owner.commit();
